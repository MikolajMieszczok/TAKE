...
public class ClubController {
    ...
    @DeleteMapping()
    public ResponseEntity<?> deleteAllClubs(@RequestParam(name="unsafe", required = false) String unsafe) {
    	...
    	if ("true".equalsIgnoreCase(unsafe)) {
	    	...
	    	try {
	    		...
	    		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("There are some references in matches, use unsafe=true or delete matches manually.");
	    	}
    	}
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteClub(@PathVariable("id") Long id, @RequestParam(name="unsafe", required = false) String unsafe) {
        Club club = ClubRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("Club not found with id: " + id));
